...
class StockCloseModelNodes:
    ...
    def evaluate_root_model_performance_from_mlflow(
        self,
        mlflow_params: dict[str, Any] | None = None,
    ) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, dict[str, Any]]:
        _apply_training_environment(
            mlflow_params=mlflow_params,
            runtime_params=None,
            mlforecast_params=None,
        )
        result = self.root_performance.evaluate_from_mlflow(
            mlflow_params=mlflow_params,
        )
        _log_step("evaluate_root_model_performance_from_mlflow", **result[-1])
        return result
