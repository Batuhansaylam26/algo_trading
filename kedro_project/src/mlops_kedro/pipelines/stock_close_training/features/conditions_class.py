...
class GapConditionCalculator:
    def __init__(self, condition_columns: list[str]) -> None:
        self.condition_columns = condition_columns

    def calculate_for_symbol(self, df_symbol: pl.DataFrame) -> pl.DataFrame:
        return df_symbol.with_columns(self.expressions()).with_columns(
            pl.col(self.condition_columns).fill_null(False)
        )
    ...
    def calculate_conditions_for_symbol(
        ...
        return GapConditionCalculator(condition_columns).calculate_for_symbol(df_symbol)
