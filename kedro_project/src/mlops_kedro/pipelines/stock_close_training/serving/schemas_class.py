...
class TimescaleSchemaManager:
    ...
    def _create_timescale_feature_table(cursor) -> None:
        ...
        column_definitions = ",\n            ".join(
            f'"{column}" {TimescaleSchemaManager._postgres_type_for_feature_column(column)}'
            for column in FEAST_OFFLINE_COLUMNS
        )
        alter_columns = ",\n            ".join(
            f'ADD COLUMN IF NOT EXISTS "{column}" {TimescaleSchemaManager._postgres_type_for_feature_column(column)}'
            for column in FEAST_OFFLINE_COLUMNS
    ...
    def _create_timescale_conventional_gap_trading_table(cursor) -> None:
        ...
        column_definitions = ",\n            ".join(
            f'"{column}" {TimescaleSchemaManager._postgres_type_for_conventional_gap_column(column)}'
            for column in CONVENTIONAL_GAP_TRADING_COLUMNS
        )
        alter_columns = ",\n            ".join(
            f'ADD COLUMN IF NOT EXISTS "{column}" {TimescaleSchemaManager._postgres_type_for_conventional_gap_column(column)}'
            for column in CONVENTIONAL_GAP_TRADING_COLUMNS
            if column not in {"symbol", "date"}
        )
        cursor.execute(
