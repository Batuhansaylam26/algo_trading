...
class PecnetSelectionBuilder:
    ...
    def _build_feature_selector_pecnet_variables(
        *,
        ...
        candidate_names, candidate_X_train, candidate_X_test = PecnetSelectionBuilder._candidate_variable_inputs(
            ticker_data
        )
        threshold = float(selection_params.get("correlation_threshold", 0.08))
        max_selected_features = PecnetSelectionBuilder._max_selected_features_for_tier(
            selection_params,
            tier_name,
        )
    ...
    def _build_residual_correlation_pecnet_variables(
        *,
        ...
        return PecnetSelectionBuilder._build_feature_selector_pecnet_variables(
            builder=builder,
    ...
    def _build_pecnet_variables(
        *,
        ...
        if strategy == "residual_correlation":
            return PecnetSelectionBuilder._build_residual_correlation_pecnet_variables(
                builder=builder,
                ticker_data=ticker_data,
                tier_name=tier_name,
                feature_selector_cls=feature_selector_cls,
