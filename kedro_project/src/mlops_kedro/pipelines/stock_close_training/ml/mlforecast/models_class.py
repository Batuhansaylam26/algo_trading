...
class MLForecastModelFactory:
    def build_auto_models(self) -> dict:
        return {
            "Ridge": AutoRidge(config=MLForecastModelFactory.ridge_config),
            "RandomForest": AutoRandomForest(config=MLForecastModelFactory.random_forest_config),
            "LightGBM": AutoLightGBM(config=MLForecastModelFactory.lightgbm_config),
            "XGBoost": AutoXGBoost(config=MLForecastModelFactory.xgboost_config),
            "CatBoost": AutoCatboost(config=MLForecastModelFactory.catboost_config),
        }

    def build_custom_auto_models(self) -> dict:
        return {
            "Ridge": AutoModel(model=Ridge(), config=MLForecastModelFactory.ridge_config),
            "RandomForest": AutoModel(
                model=RandomForestRegressor(),
                config=MLForecastModelFactory.random_forest_config,
            ),
            "LightGBM": AutoLightGBM(config=MLForecastModelFactory.lightgbm_config),
            "XGBoost": AutoXGBoost(config=MLForecastModelFactory.xgboost_config),
            "CatBoost": AutoCatboost(config=MLForecastModelFactory.catboost_config),
    ...
    def random_forest_config(trial: optuna.Trial) -> dict:
        max_estimators = MLForecastModelFactory._max_estimators()
        return {
            ...
            "random_state": 26,
            "n_jobs": MLForecastModelFactory._worker_count(),
            "verbose": 1 if MLForecastModelFactory._estimator_verbose() else 0,
        }
    ...
    def xgboost_config(trial: optuna.Trial) -> dict:
        ...
        return {
            ...
            "random_state": 26,
            "n_jobs": MLForecastModelFactory._worker_count(),
            "verbosity": 1 if MLForecastModelFactory._estimator_verbose() else 0,
        }
    ...
    def catboost_config(trial: optuna.Trial) -> dict:
        ...
        return {
            ...
            "loss_function": "RMSE",
            "random_seed": 26,
            "thread_count": MLForecastModelFactory._worker_count(),
            "verbose": MLForecastModelFactory._estimator_verbose(),
