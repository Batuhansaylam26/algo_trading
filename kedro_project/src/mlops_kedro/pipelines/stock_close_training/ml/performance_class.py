...
from .metrics import long_only_directional_metrics_by_unique_id
...
class ForecastPerformanceMeasurement:
    ...
    def measure(
        ...
        *,
        ...
        regression_df = _regression_metrics_by_unique_id(joined_df)
        long_direction_df = long_only_directional_metrics_by_unique_id(
            joined_df,
            train_df,
        )
    ...
    def log_result(
        ...
        *,
        ...
        LightweightArtifactStore().save_metrics(
            result.regression_metrics,
            f"{self.artifact_prefix}/metrics/regression_metrics.csv",
        )
        LightweightArtifactStore().save_metrics(
            result.long_direction_metrics,
            f"{self.artifact_prefix}/metrics/long_only_direction_metrics.csv",
        )
        log_forecast_plots(
            train_df=train_df,
    ...
    def _log_regression_metrics(self, regression_df: pd.DataFrame) -> None:
        for _, row in regression_df.iterrows():
            for metric_name in ["mae", "rmse", "mape", "r2"]:
                ...
                mlflow.log_metric(
                    (
                        f"{self.metric_prefix}{ForecastPerformanceMeasurement._safe_metric_component(row['unique_id'])}."
                        f"{ForecastPerformanceMeasurement._safe_metric_component(row['model'])}.test.{metric_name}"
                    ),
    ...
    def _log_long_direction_metrics(
        ...
        for _, row in long_direction_df.iterrows():
            ...
            mlflow.log_metric(
                self._long_metric_name(row, "accuracy"),
                float(row["long_accuracy"]),
            )
            mlflow.log_metric(
                self._long_metric_name(row, "precision"),
                float(row["long_precision"]),
            )
            mlflow.log_metric(
                self._long_metric_name(row, "recall"),
                float(row["long_recall"]),
            )
            mlflow.log_metric(
                self._long_metric_name(row, "signal_rate"),
                float(row["long_signal_rate"]),
            )

    def _long_metric_name(self, row: pd.Series, metric_name: str) -> str:
        return (
            f"{self.metric_prefix}{ForecastPerformanceMeasurement._safe_metric_component(row['unique_id'])}."
            f"{ForecastPerformanceMeasurement._safe_metric_component(row['model'])}.long.{metric_name}"
        )
    ...
    @staticmethod
    def _safe_metric_component(value: object) -> str:
        return str(value).replace(" ", "_").replace("/", "_").strip("_") or "unknown"
