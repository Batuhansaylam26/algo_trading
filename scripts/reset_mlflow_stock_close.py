...
def _stock_close_experiments(client: MlflowClient):
    return [
        ...
        if _should_reset_experiment(experiment.name)
    ]
...
def main() -> None:
    ...
    for registered_model in client.search_registered_models():
        if not _should_delete_registered_model(registered_model.name):
            continue
    ...
    for experiment in client.search_experiments(view_type=ViewType.DELETED_ONLY):
        if not _should_reset_experiment(experiment.name):
            continue
        client.restore_experiment(experiment.experiment_id)
        restored_experiments.append(experiment.name)

    deleted_logged_models = []
    stock_experiments = _stock_close_experiments(client)
    ...
    deleted_runs = 0
    reset_experiments = []
    for experiment in _stock_close_experiments(client):
